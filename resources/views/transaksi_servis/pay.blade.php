...
<body>
    <div class="container mt-5">
        <div class="card">
            ...
            <div class="card-body">
                <form action="{{ route('service_transactions.processPayment', $transaksi_sp->transaction_id) }}" method="POST">
                    @csrf
                    <div class="mb-3">
                        <label for="total_amount" class="form-label">Harus Dibayar</label>
                        <p class="form-control-static">Rp. {{ number_format($transaksi_sp->total_price, 0, ',', '.') }}</p>
                    </div>
                    <div class="mb-3">
    ...
    <script>
        $(document).ready(function() {
            $('#payment_amount').on('input', function() {
                var total = {{ $transaksi_sp->total_price }};
