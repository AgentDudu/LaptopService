    ...
    <div class="main-container" style="display: flex; background-color: #067D40;">
        ...
        <main class="content" style="flex: 1; padding: 20px; margin: 30px 30px 30px 0; background-color: #F8F9FA; border-radius: 20px;">
            ...
            <table class="table table-bordered">
                ...
                <tbody>
                    @foreach($service_transactions as $transaksi_sp)
                    <tr data-transaction-id="{{ $transaksi_sp->transaction_id }}">
                        <td>{{ $transaksi_sp->invoice_number }}</td>
                        <td>{{ $transaksi_sp->technician->name }}</td>
                        <td>{{ $transaksi_sp->customer->customer_name }}</td>
                        <td>{{ $transaksi_sp->laptop->laptop_brand }}</td>
                        <td>Rp {{ number_format($transaksi_sp->total_price, 2, ',', '.') }}</td>
                        <td>{{ $transaksi_sp->entry_date }}</td>
                        <td>{{ $transaksi_sp->takeout_date }}</td>
                        <td>{{ $transaksi_sp->status }}</td>
                        <td class="actions-column">
                            @if($transaksi_sp->status == 'pending')
                            <a href="{{ route('service_transactions.edit', $transaksi_sp->transaction_id) }}" class="btn btn-sm btn-warning">Edit</a>
                            <button class="btn btn-sm btn-success pay-button" data-bs-toggle="modal" data-bs-target="#payModal" data-transaction="{{ $transaksi_sp }}">Pay</button>
                            @elseif($transaksi_sp->status == 'completed')
                            <a href="{{ route('service_transactions.show', $transaksi_sp->transaction_id) }}" class="btn btn-sm btn-info">View</a>
                            @endif
                            <form action="{{ route('service_transactions.destroy', $transaksi_sp->transaction_id) }}" method="POST" style="display:inline-block;">
                                @csrf
                                @method('DELETE')
