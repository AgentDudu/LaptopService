<div class="d-flex flex-column flex-shrink-0 p-3 text-white"
    style="width: 250px; height: 100vh; background-color: #067D40">
    <style>
        .nav-link:hover,
        .nav-link.active,
    ...
    <ul class="nav nav-link-secondary flex-column fw-bold gap-2">
        <li class="nav-item">
            <a href="{{ route('dashboard') }}"
                class="nav-link text-white ps-4 {{ Route::is('dashboard.*') ? 'text-white bg-custom rounded active' : 'inactive' }}">
                @include('components.icons.svg-dashboard')
                Dashboard
            </a>
        </li>
        <li>
            <a href="#transaksi-submenu"
                class="nav-link text-white d-flex justify-content-between align-items-center {{ Route::is('transaksi.*') ? 'inactive' : '' }}"
                onclick="toggleCollapse('transaksi-submenu', this)">
                <span>
